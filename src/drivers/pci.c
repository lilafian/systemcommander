...
void function_enumerate(uint64_t device_address, uint64_t function) {
        ...
        logf("[pci:function_enumerate] %s %s - %s - %s - %s\n", 
        ...
        switch (device_header->device_class) {
                case 0x01: // Mass storage controller // TODO: ADD #DEFINES FOR THESE!!!!
...
void pci_enumerate(mcfg_header* mcfg) {
        ...
        for (int i = 0; i < entry_count; i++) {
                ...
                for (uint64_t bus = new_dev_config->start_bus; bus < new_dev_config->end_bus && bus < 64; bus++) {
